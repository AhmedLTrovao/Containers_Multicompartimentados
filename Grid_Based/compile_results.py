...
def compilar_resultados(pasta_saida):
    ...
    for arquivo in os.listdir(pasta_saida):  # percorre todos os arquivos da pasta
        if arquivo.endswith("_resumo.txt"):  # só pega os *_resumo.txt
            ...
            caixas_match = re.search(r"Número total de caixas carregadas:\s*(\d+)", conteudo)
            vol_match = re.search(r"\(([\d.,]+)% do contêiner\)", conteudo)
    ...
    excel_saida = os.path.join(pasta_saida, "Resumo_geral.xlsx")
