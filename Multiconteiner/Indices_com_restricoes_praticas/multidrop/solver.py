...
def resolver_instancia(compartimentos, clientes, arquivo_saida, tempo_limite=3600, stabv=True, stabh=False, loadbearing=False):
    ...
    # Calculando o Big-M para suportar coordenadas globais
    M = sum(comp[0] for comp in compartimentos) + 1000

    # criando variáveis
    for k, (L_k, W_k, H_k) in enumerate(compartimentos):
        # adicionar espelhamento além do offset posteriormente
        if k % 2 == 0:
            O_X[k] = 0
        else:
        ...
        #if k%2 == 0:
        #    X_coords_comp[k] = [x for x in E_L_k] # O_X[k] é zero de qualquer forma
        #else: # espelhamento aqui
        #    X_coords_comp[k] = [ L_k - x + O_X[k]for x in E_L_k]
        X_coords_comp[k] = [x + O_X[k] for x in E_L_k] # O_X[k]
            
        Y_coords_comp[k] = [y + O_Y[k] for y in E_W_k]
        Z_coords_comp[k] = E_H_k
        
        print("Coordenadas normais: ", E_L_k)
        print(k, X_coords_comp)
        ...
        for c in range(num_clientes):
            ...
            for i in demanda[c].keys():
                ...
                for (p, q, r) in vars_caixa:
                    ...
                    # 12.35
                    if c > 0:
                        model.addConstr(L_vars[k, c-1] <= p * var_x + M * (1 - var_x))
    ...
    if stabh:
        ...
        for (k, c, i, p, q, r), var in x.items():
            ...
            # --- Estabilidade X ---
            # Só exige apoio se não estiver encostada na porta
            if p > O_X[k]:
                lhs_x = gp.LinExpr()
