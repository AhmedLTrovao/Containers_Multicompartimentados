...
def resolver_instancia(compartimentos, clientes, arquivo_saida, tempo_limite=3600, 
    ...
    # =========================================================================
    # 1. Extração de Tipos Únicos, Parâmetros e Mapeamento de Clientes
    # =========================================================================
    tipos_caixas = []
    mapa_tipos = {}
    ...
    # Preserva a ordem real dos IDs vindos da Main para o Multidrop sequencial
    ...
    for cl in clientes:
        ...
        for item in cl["itens"]:
            ...
            # Assinatura única para reaproveitar o mesmo ID de tipo de caixa
    ...
    # Calculando o Big-M baseado no comprimento global máximo
    M = sum(comp[0] for comp in compartimentos) + 1000

    # =========================================================================
    # 2. Criação de Variáveis com Offsets Globais e Malhas Padrão (Sem Inversão)
    # =========================================================================
    for k, (L_k, W_k, H_k) in enumerate(compartimentos):
        ...
        # Mantendo a malha padrão da Mariana (Linear, sem inverter na geração)
        ...
        for c in lista_id_clientes:
            for i in demanda[c].keys():
                ...
                # Filtragem geométrica padrão e segura
    ...
    # Variáveis contínuas de limite do Multidrop
    L_vars = {}
    ...
    # =========================================================================
    # 4. Restrições Fundamentais (Não-Sobreposição e Demanda)
    # =========================================================================
    # 4.1. Não sobreposição (Varredura de Malha por compartimento)
    for k, (L_k, W_k, H_k) in enumerate(compartimentos):
    ...
    if stabv:
        alpha = 1.0 
        print("Gerando restricoes de Estabilidade Vertical (Z)...")
        for (k, c, i, p, q, r), var in x.items():
            if r == 0: 
                continue 
            ...
            for (k_, c_, j, p_, q_, r_), var_ in x.items():
                if k_ == k and list(lista_id_clientes).index(c_) >= list(lista_id_clientes).index(c):
                    lj, wj, hj = tipos_caixas[j]["dims"]
                    if r_ == r - hj:
                        L_ij = max(0, min(p + li, p_ + lj) - max(p, p_))
                        W_ij = max(0, min(q + wi, q_ + wj) - max(q, q_))
    ...
    # =========================================================================
    # 8. Escrita com a Estratégia de Desinversão Visual para Simetria
    # =========================================================================
        ...
        if model.SolCount > 0:
            for (k, c, i, p, q, r) in x:
                if x[k, c, i, p, q, r].X > 0.5:
                    ...
                    # --- REBATIMENTO ESPELHADO DO LADO DIREITO (ÍMPAR) ---
                    # Garante que o Cliente 0 (calculado no início X pelo solver)
                    # seja empurrado para a extremidade externa da porta direita no arquivo.
                    if k % 2 != 0: 
                        p_real = (2 * O_X[k]) + L_k - p - li
                    else:
                        p_real = p
                    
                    f.write(f"{p_real} {q} {r} {li} {wi} {hi} {i} {c} {k}\n")
    
    # =========================================================================
    # 9. Relatório de Resumo Técnico
    # =========================================================================
    ...
    with open(resumo_arquivo, "w") as f:
        ...
        if model.SolCount > 0:
            ...
            f.write("Nenhuma solucao viavel encontrada.\n")