...
def resolver_multi_compartimento(compartimentos, boxes, arquivo_saida):
    ...
    aplicando a lógica de offset (O_kx O_ky) baseada nos conjuntos X_ik, Y_ik, Z_ik.
    Coloca os compartimentos de forma transversal
    """
    m = len(boxes)
    ...
    for k, (L_k, W_k, H_k) in enumerate(compartimentos):
        if k % 2 == 0:
            ...
            O_X_k = compartimentos[k-1][0] # Pega o comprimento (L) do compartimento à sua esquerda
        ...
        X_k = [x + O_X_k for x in E_L_k] 
        Y_k = [y + O_Y_k for y in E_W_k]
        Z_k = E_H_k
    ...
    for k, (L_k, W_k, H_k) in enumerate(compartimentos):
        ...
        X_k = [x + O_X_k for x in E_L_k]
        Y_k = [y + O_Y_k for y in E_W_k] # Usando O_Y_k correto em vez de O_k
        Z_k = E_H_k
