...
def main():
    ...
    if test == "Fasano":
    ...
    elif test == "multidrop_comparison":
        L, W, H = (8, 8, 8)
        boxes = [(2,2,2), (4,4,4)]
        m = len(boxes)
        b = [(5,5), (5,5), (5,5)]
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        resolver_instancia(L, W, H, boxes, b, n, delta, os.path.join( dir_path ,'multidrop_comparison.txt'), Sigma, Peso,3600,True,True,True)
    else:
        L, W, H = (12, 8, 8)
        boxes = [(6, 3, 2), (6, 4, 3), (8, 3, 2), (4, 3, 2), (4,4,3)]
        m = len(boxes)
